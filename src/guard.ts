 ...
 */
export function isTrustedHost(host: string, trustedHosts: readonly string[]): boolean {
 ...
 */
export function networkGuard(req: IncomingMessage, trustedHosts: readonly string[] = []): string | null {
  ...
  if (origin !== undefined) {
    ...
    if (originHost !== hostUrl.host) return 'forbidden: cross-origin'
  }
