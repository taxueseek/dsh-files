...
`;
  ...
  async function restoreSurvivorChips(refs) {
    ...
    for (const ref of refs) {
      ...
      await settleFrame();
      const gained = input.state.getSnapshot().occurrences.filter((o) => o.source === SOURCE_NAME && o.ref === ref).length > before;
      if (gained) replaced += ref.length;
    }
  }
  async function insertOnce(actx, ref, label) {
    const conversation = actx.get("conversation");
    if (conversation === void 0) throw new Error("conversation service unavailable");
    const input = conversation.input.for(actx);
    const before = input.state.getSnapshot().occurrences.filter((o) => o.source === SOURCE_NAME && o.ref === ref).length;
    for (let attempt = 0; attempt < 8; attempt += 1) {
      const state = input.state.getSnapshot();
      const caret = typeof input.caretSpan === "function" ? input.caretSpan() : { start: state.draft.length, end: state.draft.length };
      actx.emit("slash/input-insert-reference", {
        reference: {
          source: SOURCE_NAME,
          ref,
  ...
  function UploadDock({ useInput, inputActions }) {
    ...
    const removeCard = (ref, offset, occurrenceId) => {
      ...
      const wasUpload = meta !== void 0;
      uploadMeta.delete(ref);
      uploadedPool.delete(ref);
      if (wasUpload && meta !== void 0) {
